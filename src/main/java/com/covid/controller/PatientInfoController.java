...
public class PatientInfoController {
    ...
    public PatientLocationDto searchPatient(@RequestParam(required = false) Long locationId, @RequestParam(required = false) Long healthProId, @RequestParam(required = false) String phoneNumber,
                                            ...
                                            @RequestParam(required = false) String isolationStatus, @RequestParam(required = false) String quarantineRequestStatus, @RequestParam(required = false) String medicalRequestStatus, @RequestParam(required = false) String suppliesRequestStatus,
                                            @RequestParam(required = false) String geofenceStatus, @RequestParam(required = false) String heartbeatStatus, @RequestParam(required = false) String healthStatusAlert) {

        if ((locationId == null || healthProId == null) && StringUtils.isBlank(phoneNumber)) {
            throw new RuntimeException("PHONE_NUMBER_MANDATORY");
        }
        return patientService.searchPatients(locationId, healthProId, phoneNumber, size, from, firstName, lastName, covid19Status, quarantineStatus,
                isolationStatus, quarantineRequestStatus, medicalRequestStatus, suppliesRequestStatus, geofenceStatus, heartbeatStatus, healthStatusAlert);
