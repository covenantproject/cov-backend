...
COMMENT ON TABLE release1.health_pro IS 'Contains basic information for all health professionals. HealthProfessional.HealthProId has the same value as User.UserId for performance reasons (to avoid another join in some cases). This table is filled only for Health Professionals. Patients will not have a record in this table unless they are a health professional working on this health project.';

COMMENT ON COLUMN release1.health_pro.health_pro_id  IS 'Unique id of a health professional. Also a foreign key to users.user_id';

COMMENT ON COLUMN release1.health_pro.supervisor_id  IS 'Supervisor of the current user, may be null. Foreign key to health_pro_id in this table';

COMMENT ON COLUMN release1.health_pro.job_id  IS 'A single health professional may have multiple jobs. Start with 1 for the first job of each person and then increment automatically in the application layer.';
...
COMMENT ON COLUMN release1.demographics.user_id IS 'Primary key. Also foreign key to user.user_id (which is the same value as patient.patient_id or health_pro.health_pro_id)';
...
ALTER TABLE release1.pat_pro_relationship ADD CONSTRAINT PatProRelationship_RelType_check CHECK (rel_type IN ('Screening official', 'Quarantine official', 'Isolation official', 'Medical official', 'Field worker', 'Other') );

COMMENT ON TABLE release1.pat_pro_relationship IS 'Relationship between a patient and a health professional (also known as health care provider or simply ''provider'') when they provide care to a patient. Care in this context includes quarantine/isolation service to the patient. Only providers with a patient-provider relationship can view personally identifiable information or protected health information of a patient through the health professional dashboard. Other health professionals can only view summary statistics and reports.';

COMMENT ON COLUMN release1.pat_pro_relationship.rel_type IS 'Type of relationship. Constrained by CHECK CONSTRAINT.';

COMMENT ON COLUMN release1.pat_pro_relationship.rel_start_date IS 'Optional.';

COMMENT ON COLUMN release1.pat_pro_relationship.rel_end_date IS 'Optional.';

COMMENT ON COLUMN release1.pat_pro_relationship.rel_facility_location IS 'Foreign key to Location that is pertinent to this relationship. This may be a local health department or a hospital, etc.';
